...
public interface ConnectionRepository extends JpaRepository<Connection, UUID> {
    ...
    Optional<Connection> findByRequesterIdAndReceiverIdOrReceiverIdAndRequesterId(UUID requesterId, UUID receiverId, UUID receiverId2, UUID requesterId2);

    List<Connection> findAllByStatusAndRequesterIdOrStatusAndReceiverId(ConnectionStatus status1,UUID userId1, ConnectionStatus status2,UUID userId2);
    ...
    Optional<Connection> findByRequesterIdAndReceiverIdAndStatus(UUID requesterId, UUID receiverId, ConnectionStatus status);
